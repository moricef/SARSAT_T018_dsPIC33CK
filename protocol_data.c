...
// ELT state
elt_state_2g_t elt_state_2g = {.current_phase = ELT_PHASE_1,
...
static void write_bits(uint8_t *bit_array, int start_pos, int num_bits,
                       uint64_t value) {
...
void build_2g_information_field(uint8_t *info_bits) {
  ...
  // Logging disabled to avoid UART blocking before LED ON
  ...
  uint16_t lat_decimal_encoded =
      (uint16_t)(lat_decimal * 32768.0 + 0.5); // Round
  ...
  uint16_t lon_decimal_encoded =
      (uint16_t)(lon_decimal * 32768.0 + 0.5); // Round
  ...
  // Bits 124-137: EPIRB-AIS System Identity (14 bits) - spare for basic
  // implementation
  ...
  uint16_t spare_bits =
      (beacon_config_2g.rotating_type == RF_TYPE_CANCEL_2G) ? 0x3FFF : 0x3FFF;
...
void set_rotating_field_2g(uint8_t *info_bits,
                           rotating_field_type_2g_t rf_type) {
  ...
  case RF_TYPE_G008_2G:
    // T.018 G.008 Objective Requirements rotating field
    {
      ...
      if (beacon_config_2g.test_mode) {
        uint8_t lfsr_state = elt_state_2g.transmission_count &
                             0xFF; // Seed with transmission count
        for (int i = 0; i < 27;
             i++) { // 27 bits for rotating field (T.018 bits 186-202)
          lfsr_state = lfsr_8bit(lfsr_state);
          info_bits[185 + i] = lfsr_state & 0x01; // Set LSB of LFSR state
        }
      }
      // Common fields for both Test and Exercise modes
    ...
    /*        case RF_TYPE_G008_2G:
                ...
                {
                    uint8_t elapsed_hours = get_elapsed_activation_hours_2g();
                    uint16_t last_pos_minutes =
       get_time_since_last_location_minutes_2g();

                    write_bits(info_bits, 158, 6, elapsed_hours);           //
       T.018 bits 159-164 write_bits(info_bits, 164, 11, last_pos_minutes); //
       T.018 bits 165-175 write_bits(info_bits, 175, 10, rf_data.altitude_code);
       // T.018 bits 176-185 write_bits(info_bits, 185, 17, 0); // T.018 bits
       186-202
                }
                break;
    */
...
uint32_t encode_time_value_2g(uint8_t day, uint8_t hour, uint8_t minute,
                              uint8_t type) {
  switch (type) {
  case RF_TYPE_G008_2G:
  case RF_TYPE_ELTDT_2G:
...
beacon_config_2g_t *get_beacon_config_2g(void) { return &beacon_config_2g; }
...
uint32_t get_current_interval_2g(void) {
  ...
  case ELT_PHASE_3:
    ...
    return ELT_PHASE3_INTERVAL + (rand() % (ELT_PHASE3_RANDOM * 2)) -
           ELT_PHASE3_RANDOM;
  default:
...
uint16_t get_time_since_last_location_minutes_2g(void) {
  ...
  if (elapsed_minutes > 2046)
    elapsed_minutes = 2046;
  ...
  case RF_TYPE_ELTDT_2G: {
    ...
    if (gps && gps->valid) {
      rf_data->time_value = encode_time_value_2g(
          gps->day, gps->hour, gps->minute, beacon_config_2g.rotating_type);
...
uint64_t get_rls_data_2g(void) { return 0; }
...
void test_rotating_field_compliance(void) {
    uint8_t test_frame1[202] = {0};
    uint8_t test_frame2[202] = {0};

    // Save current system state
    uint16_t saved_count = elt_state_2g.transmission_count;
    uint8_t saved_test_mode = beacon_config_2g.test_mode;
    rotating_field_type_2g_t saved_rotating_type = beacon_config_2g.rotating_type;

    // Force Test Mode for this test
    beacon_config_2g.test_mode = 1;
    beacon_config_2g.rotating_type = RF_TYPE_G008_2G;

    // Simulate two consecutive transmissions
    elt_state_2g.transmission_count = 0;
    build_2g_information_field(test_frame1);

    elt_state_2g.transmission_count = 1;
    build_2g_information_field(test_frame2);

    // Check rotating field bits (185-202) for differences
    int differences = 0;
    for (int i = 185; i < 202; i++) {
        if (test_frame1[i] != test_frame2[i]) {
            differences++;
        }
    }

    // Restore previous system state
    elt_state_2g.transmission_count = saved_count;
    beacon_config_2g.test_mode = saved_test_mode;
    beacon_config_2g.rotating_type = saved_rotating_type;

    DEBUG_LOG_FLUSH("Rotating Field Test: ");
    if (differences == 27) {
        DEBUG_LOG_FLUSH("PASS (27/27 bits changed)\r\n");
    } else {
        DEBUG_LOG_FLUSH("FAIL (");
        debug_print_dec(differences);
        DEBUG_LOG_FLUSH("/27 bits changed)\r\n");
    }
}
